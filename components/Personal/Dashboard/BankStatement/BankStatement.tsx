"use client";

import React, { useState } from "react";
import DashboardLayout from "../DashboardLayout/DashboardLayout";
import { URLS } from "@/lib/constants/url";
import InputDetails from "./TabsPage/InputDetails";
import BankStatementTable from "./TabsPage/BankStatementTable";
import Button from "@/components/ui/Button/Button";
import { useForm, FormProvider } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import {
  addBeneficiaryForm,
  addBeneficiarySchema,
} from "@/lib/schemas/transferschema";
import { useTransfer } from "@/lib/hooks/Personal/Dashboard/Transfer/useTransfer";

import "../../../ui/Transformation.css";

const BankStatement = () => {
  const [step, setStep] = useState(1);
  const [isTransitioning, setIsTransitioning] = useState(false);
  const { transferToOwnAccount } = useTransfer();

  const methods = useForm<addBeneficiaryForm>({
    resolver: zodResolver(addBeneficiarySchema),
    mode: "onChange",
    reValidateMode: "onChange",
    defaultValues: {
      accountName: "",
      bank: "",
      accountNumber: "",
    },
  });

  const nextStep = async () => {
    if (step === 1) {
      const isValid = await methods.trigger();
      if (!isValid) return;
    }

    if (step < 2) {
      setIsTransitioning(true);
      setTimeout(() => {
        setStep((prev) => prev + 1);
        setIsTransitioning(false);
      }, 150);
    }
  };

  const prevStep = () => {
    if (step > 1) {
      setIsTransitioning(true);
      setTimeout(() => {
        setStep((prev) => prev - 1);
        setIsTransitioning(false);
      }, 150);
    }
  };

  const handleFinalSubmit = () => {
    const formData = methods.getValues();

    // Uncomment below when ready to send transfer mutation
    /*
    transferToOwnAccount.mutate(
      {
        ...formData,
        // otp: otpValue, // include if needed
      },
      {
        onSuccess: () => {
          nextStep(); // Go to final step/success screen
        },
        onError: (error) => {
          console.error("Transfer failed:", error);
        },
      }
    );
    */

    console.log("Submitted Form Data:", formData);
    nextStep(); // Remove this line if using mutate above
  };

  const renderStepContent = () => {
    switch (step) {
      case 1:
        return <InputDetails />;
      case 2:
        return <BankStatementTable />;
      default:
        return null;
    }
  };

  return (
    <DashboardLayout urlpath={URLS.DASHBOARD.PERSONAL.TRANSFER}>
      <h1 className="font-bold text-xl">Bank Statement</h1>

      <div className="bg-white border mt-8 border-gray-200 px-14 rounded-xl transfer-form-container transfer-form">
        {step < 2 && (
          <div className="border-b py-6 border-gray-300 flex font-bold justify-between">
            <p className="step-title">
              {step === 1 ? "Bank Statement" : "Enter Transaction Pin"}
            </p>
            <p>Step {step} of 2</p>
          </div>
        )}

        <FormProvider {...methods}>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              step < 2 ? nextStep() : handleFinalSubmit();
            }}
          >
            <div className="py-6">
              <div className={`step-content ${isTransitioning ? "transitioning" : ""}`}>
                {renderStepContent()}
              </div>
            </div>

            {step < 2 && (
              <div className="flex justify-between py-4">
                {step > 1 ? (
                  <button
                    type="button"
                    onClick={prevStep}
                    className="px-6 py-2 bg-gray-200 rounded hover:bg-gray-300"
                  >
                    Back
                  </button>
                ) : (
                  <div />
                )}

                <Button
                  type="submit"
                  className="text-white bg-primary-02 hover:bg-primary"
                  disabled={transferToOwnAccount.isPending}
                >
                  {step < 2 ? "Continue" : "Proceed to OTP"}
                </Button>
              </div>
            )}
          </form>
        </FormProvider>
      </div>
    </DashboardLayout>
  );
};

export default BankStatement;
